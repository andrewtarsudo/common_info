...
```
{
  "login": "логин_на_YouTrack",
  "auth_token": "perm:автоматически_сгенерированный_токен",
...
* `timestamp` - временная метка последнего использования программы в формате ISO: ГГГГ-ММ-ДД (2022-01-25). После успешного запуска значение меняется на текущий день. 
...
{
  "login": "tarasov-a",
  "auth_token": "perm:dGFyYXNvdi1h.NjEtMTM2.iApcbQQA7dqNozSoUcL38qL6gvNgdx",
...
-------- (3.1) если указанный файл не существует, то он будет создан, а в него будет записаны результаты. После оповещения времени программа прекратит работу, а окно закроется.

-------- (3.2) если указанный файл уже существует, то будет запрошено подтверждение перезаписи. По умолчанию - отмена перезаписи.
...
------------ (3.2.1) если перезапись отменена, то осуществляется возврат к пункту (3).
...
* взаимодействие с информацией из запросов:
 ...
 Если есть идеи, предложения, то обращаться по контактаам [ниже](#контакты)
