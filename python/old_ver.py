...
def text_extr(file_path, filename):  # извлечь текст
    ...
    while True:
         # если файл pdf
        if pathlib.PurePath(full_path).suffix == '.pdf':
            ...
            break
        # если файл txt
        elif pathlib.PurePath(full_path).suffix == '.txt':
            try:
        ...
        # если расширение пустое, т.е. выбрана директория
        elif pathlib.PurePath(full_path).suffix == '':
...
# проверка, что текст удалось извлечь
...
def find_str(text, find_words):  # поиск подстроки в тексте
    list_ok = []  # список строк, в которых содержатся искомые слова
